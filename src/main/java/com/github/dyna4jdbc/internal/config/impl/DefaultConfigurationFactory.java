	...
	public Configuration newConfigurationFromParameters(String config, Properties props) throws SQLException {
		...
		for (Object propKey : props.keySet()) {
			if (!(propKey instanceof java.lang.String)) {
				throw SQLError.INVALID_CONFIGURATION.raiseException("properties should only contain String keys!");
			} else {
				String key = (String) propKey;

				if (internalPropops.containsKey(key)) {
					throw SQLError.INVALID_CONFIGURATION
							.raiseException("duplicated configuration between JDBC URL and properties: %s", key);
				}
		...
		try {
			...
			throw SQLError.INVALID_CONFIGURATION.raiseException("Failed to apply configuration: " + ex.getMessage());
		}
